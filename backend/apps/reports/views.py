...
class DashboardSummaryView(APIView):
    def get(self, request):
        ...
        try:
            ...
            # Use all_with_deleted() then filter alive, so we get the correct queryset
            # Item.objects already excludes soft-deleted via SoftDeleteManager
            items = Item.objects.all()
            stock_value = sum(item.available_value() for item in items)
            stock_items_count = items.count()

            # Build expiry alerts from active trucks
            ...
            # Trips this month: exclude CANCELLED, only count real trips
