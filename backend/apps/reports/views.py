...
class DashboardSummaryView(BranchFilterMixin, APIView):
    ...
    def _truck_breakdown(self, date_from, date_to, branch_id=None):
        ...
        for truck in qs:
            # FIX: explicit branch filter as defense-in-depth, same reasoning
            # as TruckWiseSummaryView above.
            trips = Trip.objects.filter(
                truck=truck,
                loading_time__date__gte=date_from,
                loading_time__date__lte=date_to,
            )
            exp_base = Expenditure.objects.filter(
                truck=truck, date__gte=date_from, date__lte=date_to
            )
            if truck.branch_id:
                trips = trips.filter(branch_id=truck.branch_id)
                exp_base = exp_base.filter(branch_id=truck.branch_id)
            trip_rev  = _fmt(trips.aggregate(t=Sum('trip_revenue'))['t'])
            total_exp = _fmt(exp_base.aggregate(t=Sum('amount'))['t'])
...
class TruckWiseSummaryView(BranchFilterMixin, APIView):
    def get(self, request):
        ...
        rows = []
        for truck in qs:
            # FIX: filter explicitly by truck.branch_id (not just truck=truck)
            # as defense-in-depth. If a Trip/Revenue/Expenditure row was ever
            # mis-stamped with the wrong branch (e.g. branch=NULL from before
            # the _sync_finance fix), this keeps it from bleeding into a
            # different branch's truck-wise summary.
            truck_branch_id = truck.branch_id
            ...
            if truck_branch_id:
                trips = trips.filter(branch_id=truck_branch_id)
            trip_count = trips.count()
            trip_rev   = _fmt(trips.aggregate(t=Sum('trip_revenue'))['t'])
            other_rev_qs = Revenue.objects.filter(
                date__gte=date_from, date__lte=date_to, trip__truck=truck,
            ).exclude(source='TRIP_REVENUE')
            if truck_branch_id:
                other_rev_qs = other_rev_qs.filter(branch_id=truck_branch_id)
            other_rev  = _fmt(other_rev_qs.aggregate(t=Sum('amount'))['t'])
            total_rev  = round(trip_rev + other_rev, 2)
            exp_qs = Expenditure.objects.filter(truck=truck, date__gte=date_from, date__lte=date_to)
            if truck_branch_id:
                exp_qs = exp_qs.filter(branch_id=truck_branch_id)
            def _exp_cat(cat):
