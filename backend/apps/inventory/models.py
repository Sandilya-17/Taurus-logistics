...
FIX: Added `branch` ForeignKey to StockLedger so every transaction is tagged
to the branch that made it.  This eliminates cross-branch stock leakage.
"""
...
class Item(SoftDeleteModel):
    ...
    def available_qty(self, location=None, as_of=None, branch=None):
        """Dynamically calculate current stock from ledger, optionally per-branch."""
    ...
    def available_value(self, location=None, as_of=None, branch=None):
        """Dynamically calculate current stock value from ledger, optionally per-branch."""
        qs = StockLedger.objects.filter(item=self)
        if location:
            qs = qs.filter(location=location)
        if as_of:
...
class StockLedger(TimeStampedModel):
    ...
    Closing stock = SUM(quantity)  — always filtered by branch!
    Closing value = SUM(final_amount)

    NEVER store closing stock as a field. Always compute dynamically.

    FIX: `branch` column added so each row is branch-scoped.
    """
    OPENING      = 'OPENING'
    PURCHASE     = 'PURCHASE'
    ISSUE        = 'ISSUE'
    ...
    branch = models.ForeignKey(
        'users.Branch', null=True, blank=True,
        on_delete=models.PROTECT, related_name='+', db_index=True,
        help_text='Branch that owns this stock transaction',
    )
    # ─────────────────────────────────────────────────────────────────────────
    ...
    class Meta:
        ...
        ordering = ['-created_at']
        indexes  = [
            models.Index(fields=['branch', 'item', 'location', 'created_at']),
